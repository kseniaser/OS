...
int main(){
    ...
    socklen_t addr_size;
    clientSocket = socket(PF_INET, SOCK_DGRAM, 0); //создаем UDP сервер
    ...
    while(1){
        ...
        fgets(buffer,1024,stdin); //считываем
        printf("You typed: %s",buffer); //печатаем для проверки
